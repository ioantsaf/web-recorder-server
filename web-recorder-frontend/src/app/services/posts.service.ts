...
export class PostsService {
  serverURL = 'http://snf-750380.vm.okeanos.grnet.gr:4000/users/';

  constructor(private http: Http) { }

  getAllUsers() {
    return this.http.get(this.serverURL, this.jwt()).map((response: Response) => response.json());
  }
  ...
  getSuite(_id: string, suiteName: string) {
    return this.http.get(this.serverURL + _id + '/' + suiteName, this.jwt()).map((response: Response) => response.json());
  }

  getTests(_id: string, suiteName: string) {
    // tslint:disable-next-line:max-line-length
    return this.http.get(this.serverURL + _id + '/' + suiteName + '/tests', this.jwt()).map((response: Response) => response.json());
  }

  getTest(_id: string, suiteName: string, testName: string) {
    // tslint:disable-next-line:max-line-length
    return this.http.get(this.serverURL + _id + '/' + suiteName + '/tests/' + testName, this.jwt()).map((response: Response) => response.json());
  }

  getTestResult(_id: string, suiteName: string, testName: string) {
    // tslint:disable-next-line:max-line-length
    return this.http.get(this.serverURL + _id + '/' + suiteName + '/tests/' + testName + '/result', this.jwt()).map((response: Response) => response.json());
  }

  getSuiteHistory(_id: string, suiteName: string) {
    // tslint:disable-next-line:max-line-length
    return this.http.get(this.serverURL + _id + '/' + suiteName + '/history', this.jwt()).map((response: Response) => response.json());
  }

  getSuiteTestsHistory(_id: string, suiteName: string, historyID: string) {
    // tslint:disable-next-line:max-line-length
    return this.http.get(this.serverURL + _id + '/' + suiteName + '/history/' + historyID, this.jwt()).map((response: Response) => response.json());
  }

  getSuiteTestHistory(_id: string, suiteName: string, historyID: string, testName: string) {
    // tslint:disable-next-line:max-line-length
    return this.http.get(this.serverURL + _id + '/' + suiteName + '/history/' + historyID + '/' + testName, this.jwt()).map((response: Response) => response.json());
  }

  getTestHistory(_id: string, suiteName: string, testName: string, historyID: string) {
    // tslint:disable-next-line:max-line-length
    return this.http.get(this.serverURL + _id + '/' + suiteName + '/tests/' + testName + '/history/' + historyID, this.jwt()).map((response: Response) => response.json());
  }
  ...
  updateTest(_id: string, suiteName: string, testName: string, info: any) {
    return this.http.put(this.serverURL + _id + '/' + suiteName + '/tests/' + testName + '/edit', info, this.jwt());
  }

  updateTestResult(_id: string, suiteName: string, testName: string, result: any[]) {
    // tslint:disable-next-line:max-line-length
    return this.http.put(this.serverURL + _id + '/' + suiteName + '/tests/' + testName + '/update-result', result, this.jwt());
  }

  updateTestSettings(_id: string, suiteName: string, testName: string, info: any) {
    return this.http.put(this.serverURL + _id + '/' + suiteName + '/tests/' + testName + '/settings', info, this.jwt());
  }

  updateSuiteSettings(_id: string, suiteName: string, info: any) {
    return this.http.put(this.serverURL + _id + '/' + suiteName + '/settings', info, this.jwt());
  }

  updateSuiteHistory(_id: string, suiteName: string) {
    return this.http.put(this.serverURL + _id + '/' + suiteName + '/history', {}, this.jwt());
  }

  updateStepsID(_id: string, suiteName: string, testName: string, index: number, newID: string) {
    // tslint:disable-next-line:max-line-length
    return this.http.put(this.serverURL + _id + '/' + suiteName + '/tests/' + testName + '/updateid', { 'index': index, 'newID': newID }, this.jwt());
  }

  deleteTest(_id: string, suiteName: string, testName: string) {
    return this.http.put(this.serverURL + _id + '/' + suiteName + '/tests/' + testName, {}, this.jwt());
  }

  deleteSuite(_id: string, suiteName: string) {
    return this.http.put(this.serverURL + _id + '/' + suiteName, {}, this.jwt());
  }

  deleteSuiteHistory(_id: string, suiteName: string, historyID: string) {
    return this.http.put(this.serverURL + _id + '/' + suiteName + '/history/' + historyID, {}, this.jwt());
  }

  deleteSuiteTestHistory(_id: string, suiteName: string, historyID: string, testName: string) {
    return this.http.put(this.serverURL + _id + '/' + suiteName + '/history/' + historyID + '/' + testName, {}, this.jwt());
  }

  deleteTestHistory(_id: string, suiteName: string, testName: string, historyID: string) {
    // tslint:disable-next-line:max-line-length
    return this.http.put(this.serverURL + _id + '/' + suiteName + '/tests/' + testName + '/history/' + historyID, {}, this.jwt());
  }
